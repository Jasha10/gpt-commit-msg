    ...
    for part in parts[1:]:
        ...
        if chunk_tcount + part_tcount >= max_token_count[args.model]:
            ...
            if llm.get_num_tokens(text) > max_token_count[args.model]:
                ...
                summaries.extend(
                    summarize(llm, text, splitre=splitre[1:], prompt=prompt)
                )
            else:
...
def main():
    ...
    def parse_temperature(value: str) -> float:
        ...
        raise argparse.ArgumentTypeError(
            f"Temperature must be between 0 and 2, not {value}"
        )
    ...
    parser.add_argument(
        "--quiet",
        "-q",
        help="Suppress printing of cache hit counter info",
        action="store_true",
    )
    parser.add_argument(
    ...
    if args.logfile:
        logging.basicConfig(
            filename=os.path.expanduser(args.logfile), level=logging.INFO
        )

    logging.info(f"Got args: {args}")

    llm = llmlib.Llm(
        llmlib.Openai(model=args.model, temperature=args.temperature),
